// avaliacao-modal.js - Modal de avaliação

function abrirModalAvaliacao(tipo, id) {
    // Buscar o nome do estabelecimento
    var card = document.querySelector('.' + tipo + '-card[data-id="' + id + '"]');
    var nome = card ? card.querySelector('h3')?.textContent || 'Estabelecimento' : 'Estabelecimento';
    ...
    var estrelas = modal.querySelectorAll('.estrela-modal');
    ...
    for (var i = 0; i < estrelas.length; i++) {
        estrelas[i].addEventListener('click', function() {
            ...
            for (var j = 0; j < estrelas.length; j++) {
                if (j < notaSelecionada) {
                    estrelas[j].textContent = '★';
                    estrelas[j].style.color = '#fbbf24';
                } else {
                    estrelas[j].textContent = '☆';
                    estrelas[j].style.color = '#d1d5db';
                }
            }
        });
    }
    
    window.notaModalSelecionada = function() { return notaSelecionada; };
}

function fecharModalAvaliacao() {
    var modal = document.querySelector('.avaliacao-modal');
    if (modal) modal.remove();
}

async function enviarAvaliacaoModal(tipo, id) {
    var nome = document.getElementById('avaliacao-modal-nome').value.trim();
    var email = document.getElementById('avaliacao-modal-email').value.trim();
    var comentario = document.getElementById('avaliacao-modal-comentario').value.trim();
    var nota = window.notaModalSelecionada ? window.notaModalSelecionada() : 0;
    ...
    try {
        ...
        var response = await fetch(API_URL + '/avaliacoes', {
            method: 'POST',
...
(function adicionarEstilosModal() {
    if (document.querySelector('#style-avaliacao-modal')) return;
    
    var style = document.createElement('style');
    style.id = 'style-avaliacao-modal';
    style.textContent = `
        .avaliacao-modal {
            position: fixed;
            top: 0;
