// load-farmacias.js - Versão corrigida (sem recarregamento infinito)

// Variável de controle para evitar carregamento duplicado
let farmaciasCarregadas = false;
let isLoading = false;
...
async function carregarFarmacias() {
    ...
    if (farmaciasCarregadas || isLoading) {
        console.log("Farmácias já carregadas ou carregando...");
        return;
    }
...
// Função para obter direções via Google Maps
function obterDirecoes(endereco) {
    if (!endereco || endereco === 'Endereço não informado') {
        alert('Endereço não disponível para esta farmácia.');
        return;
    }
    
    const enderecoCompleto = encodeURIComponent(endereco + ', Nampula, Moçambique');
    const url = `https://www.google.com/maps/dir/?api=1&destination=${enderecoCompleto}`;
    window.open(url, '_blank');
}

// Função para ver no mapa (Google Maps)
...
function abrirWaze(endereco) {
    ...
    const enderecoCompleto = encodeURIComponent(endereco + ', Nampula, Moçambique');
    const url = `https://www.waze.com/ul?q=${enderecoCompleto}&navigate=yes`;
    window.open(url, '_blank');
}
...
function criarCardFarmacia(farmacia) {
    ...
    // Escapar o endereço para evitar problemas com aspas
    const enderecoEscapado = endereco.replace(/'/g, "\\'").replace(/"/g, '&quot;');
    ...
    card.innerHTML = `
        ...
        <div class="directions-container" style="display: flex; gap: 8px; margin-top: 10px;">
            <button class="directions-btn" onclick="obterDirecoes('${enderecoEscapado}')" style="flex: 1; background: #4285F4; color: white; border: none; padding: 8px; border-radius: 8px; font-size: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 5px;">
                🗺️ Como Chegar
            </button>
            <button class="waze-btn" onclick="abrirWaze('${enderecoEscapado}')" style="flex: 1; background: #33CCFF; color: white; border: none; padding: 8px; border-radius: 8px; font-size: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 5px;">
                🚗 Waze
            </button>
        </div>
    `;
...
async function carregarMedicamentosDaFarmacia() {
    ...
    try {
        const farmacias = await apiRequest('/farmacias');
        let farmacia = farmacias.find(f => f.id == id || f.nome === nomeDecodificado);
        ...
        const enderecoEscapado = farmacia ? farmacia.endereco.replace(/'/g, "\\'") : '';
        ...
        medicamentosHTML += `
                ...
                <div style="display: flex; gap: 10px; margin-top: 20px;">
                    <button onclick="obterDirecoes('${enderecoEscapado}')" style="flex: 1; background: #4285F4; color: white; border: none; padding: 12px; border-radius: 8px; cursor: pointer;">
                        🗺️ Como Chegar (Google Maps)
                    </button>
                    <button onclick="abrirWaze('${enderecoEscapado}')" style="flex: 1; background: #33CCFF; color: white; border: none; padding: 12px; border-radius: 8px; cursor: pointer;">
                        🚗 Abrir no Waze
                    </button>
                </div>
...
async function carregarDetalhesDaFarmacia() {
    ...
    try {
        ...
        const produtos = await apiRequest(`/produtos?farmaciaId=${farmacia.id}`);
        ...
        let googleMapsDirectionsLink = `https://www.google.com/maps/dir/?api=1&destination=${enderecoParaMapa}`;
        
        let avaliacao = (Math.random() * 2 + 3).toFixed(1);
        let estrelas = '';
        for (let i = 0; i < 5; i++) {
            if (i < Math.floor(avaliacao)) estrelas += '⭐';
            else if (i < avaliacao) estrelas += '✨';
            else estrelas += '☆';
        }
                    ...
                    <div>
                        <div style="background: #f9fafb; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                            ...
                            <div style="display: flex; gap: 10px;">
                                <a href="${googleMapsLink}" target="_blank" style="flex: 1; background: #4285F4; color: white; text-decoration: none; padding: 10px; border-radius: 8px; text-align: center;">🗺️ Ver no Mapa</a>
                                <a href="${googleMapsDirectionsLink}" target="_blank" style="flex: 1; background: #ea4335; color: white; text-decoration: none; padding: 10px; border-radius: 8px; text-align: center;">🧭 Como Chegar</a>
                                <a href="${wazeLink}" target="_blank" style="flex: 1; background: #33CCFF; color: white; text-decoration: none; padding: 10px; border-radius: 8px; text-align: center;">🚗 Waze</a>
                            </div>
                        </div>
                    ...
                    <div>
                        <div style="background: #f9fafb; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                            ...
                            <div style="text-align: center;">
                                <div style="font-size: 24px;">${estrelas}</div>
                                <div style="font-size: 20px; font-weight: bold;">${avaliacao}</div>
                                <div style="color: #6b7280;">de 5 estrelas</div>
                                <div style="margin-top: 10px; color: #7c3aed;">📊 150+ avaliações</div>
                            </div>
                        </div>
                        ...
                        <div style="background: #f9fafb; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                                <div style="text-align: center; background: white; padding: 15px; border-radius: 8px;">
                                    ...
                                    <div style="font-size: 12px;">Produtos</div>
                                </div>
                                <div style="text-align: center; background: white; padding: 15px; border-radius: 8px;">
                                    <div style="font-size: 24px; color: #7c3aed;">${Math.floor(Math.random() * 50 + 20)}</div>
                                    <div style="font-size: 12px;">Clientes/dia</div>
                                </div>
                            </div>
                        </div>
...
window.obterDirecoes = obterDirecoes;
window.verNoMapa = verNoMapa;
window.abrirWaze = abrirWaze;
window.filtrarFarmacias = filtrarFarmacias;

// Garantir que a inicialização ocorre apenas uma vez
let inicializado = false;

document.addEventListener('DOMContentLoaded', function () {
    if (inicializado) {
        console.log("Inicialização já foi feita, ignorando...");
        return;
    }
    inicializado = true;
    
    const path = window.location.pathname;
    console.log("Inicializando página:", path);
