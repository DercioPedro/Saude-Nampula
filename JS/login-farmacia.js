// load-farmacias.js - Versão sem geolocalização
...
// ==================== FUNÇÕES DE DIREÇÕES (SEM GEOLOCALIZAÇÃO) ====================

// Função para obter direções (Google Maps - abre com rota)
function obterUrlDirecoes(item) {
    ...
    // Abrir diretamente com direções (sem origem, o Google Maps pede a localização)
...
// ==================== FUNÇÕES PARA ABRIR DIREÇÕES ====================

function abrirDirecoes(farmaciaId) {
    // Buscar a farmácia da lista global ou da API
    const card = document.querySelector(`.farmacia-card[data-id="${farmaciaId}"]`);
    if (!card) {
    ...
    const endereco = card.querySelector('.detail-item span')?.textContent || '';
    const nome = card.querySelector('h3')?.textContent || 'Farmácia';
    
    if (!endereco || endereco === 'Endereco nao informado') {
        alert('Esta farmácia não tem endereço cadastrado.');
        return;
    }
    
    // Gerar URL
    const destino = encodeURIComponent(endereco + ', Nampula, Moçambique');
    const url = `https://www.google.com/maps/dir/?api=1&destination=${destino}&travelmode=driving`;
    
    console.log('🔗 Abrindo direções para:', endereco);
    console.log('🔗 URL:', url);
    
    window.open(url, '_blank');
}

function abrirWaze(farmaciaId) {
    const card = document.querySelector(`.farmacia-card[data-id="${farmaciaId}"]`);
    if (!card) {
        alert('Farmácia não encontrada!');
        return;
    ...
    console.log('🔗 Abrindo Waze para:', endereco);
    window.open(url, '_blank');
}
